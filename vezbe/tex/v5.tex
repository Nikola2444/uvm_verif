...
Sa slike \ref{fig:v5_vivado_set_uvm_variables} može se videti da u polje \emph{xsim.simulation.xsim.more\(\_\)options*} treba da
...
\lstinputlisting[caption=v5\(\_\)calc\(\_\)test\(\_\)pkg, label=lst:v5_calc_test_pkg]{code/v5_calc_test_pkg.sv}
\lstinputlisting[caption=v5\(\_\)calc\(\_\)agent\(\_\)pkg, label=lst:v5_calc_agent_pkg]{code/v5_calc_agent_pkg.sv}
...
\lstinputlisting[caption=v5\(\_\)calc\(\_\)seq\(\_\)item, label=lst:v5_calc_seq_item]{code/v5_calc_seq_item.sv}
